...
examples/output: $(SOURCE_SCHEMA_PATH)
	...
	$(RUN) linkml-run-examples \
		--output-formats json \
		--output-formats yaml \
		--input-directory tests/input/examples \
		--output-directory $@ \
		--schema $< > $@/README.md
...
git-init:
	git init
git-add: .cruft.json
	git add .gitignore .github .cruft.json Makefile LICENSE *.md examples utils about.yaml mkdocs.yml poetry.lock project.Makefile pyproject.toml src/hoopla/schema/*yaml src/*/datamodel/*py src/data src/docs tests
